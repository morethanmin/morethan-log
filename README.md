...
- Customize `blog.config.js`을 본인에게 맞도록 수정해주세요.
  - pageId같은 경우에는 notion config의 pageId는 본인의 notion에 [해당 페이지](https://quasar-season-ed5.notion.site/12c38b5f459d4eb9a759f92fba6cea36?v=2e7962408e3842b2a1a801bf3546edda)를 복제를 한뒤, 해당 페이지에서 `공유` -> `웹에서 공유` 클릭시 생성되는 url(www.notion.so/[pageId]?v=[version])에서 pageId를 입력해주시면 됩니다.
